...
function init() {
  ...
  function drawText() {
    ...
    ctx.fillText("I'm so proud of you ell", width / 2, height / 2.2 + 400);
  }
...
function continueMusic() {
  ...
  // Autoplay: file ini akan autoplay dalam kondisi muted (biar tidak ditolak browser).
  // Setelah user klik, baru kita unmute.
  if (isMusicPlaying) {
    music.currentTime = parseFloat(musicCurrentTime);
  }

  // Coba play (biasanya sudah autoplay kalau atribut autoplay+muted ada)
  music.play().catch((error) => console.log("Music playback failed", error));

  const tryPlay = () => {
    // Pastikan unmute + volume tidak 0
    music.muted = false;
    music.volume = Math.max(0.8, music.volume || 0.8);
  ...
  // Hindari nambah event listener berkali-kali (di beberapa kondisi script bisa terpanggil ulang)
  document.removeEventListener("click", tryPlay);
  document.addEventListener("click", () => {
    const ok = tryPlay();
    if (ok) document.removeEventListener("click", () => {});
  }, { once: true });
}
